...
#Authors: Lutz Gschwind & Romy Nefs
#Last changed: 2024-01-25
...
IMPIC_2016 <- read_excel("MIND.xlsx", sheet = "IMPIC_2016")
IMPIC_Politcal_Rights <- read_excel("MIND.xlsx", sheet = "IMPIC_Political_Rights")
DEMIG_Policy <- read_excel("MIND.xlsx", sheet = "DEMIG_Policy")
...
DEMIG_Policy <- substitute_characters(DEMIG_Policy)
...
IMPIC_2016 <- substitute_characters(IMPIC_2016)
...
hierarical_structure <- function(data_frame, variable_name) {
  ...
  if (target$Type[1] == "Index") {
    return(paste0(
      "\\subsection{", target$Title[1], " (", tolower(target$Dataset[1]), "_", tolower(target$Code[1]), ")}\n",
      description_text
    ))
    ...
    return(paste0(
      "\\subsubsection{", target$Title[1], " (", tolower(target$Dataset[1]), "_", tolower(target$Code[1]), ")}\n",
      description_text
    ))
    ...
    return(paste0(
      "\\subsubsubsection{", target$Title[1], " (", tolower(target$Dataset[1]), "_", tolower(target$Code[1]), ")}\n",
      description_text
    ))
    ...
    return(paste0(
      "\\subsubsubsubsection{", target$Title[1], " (", tolower(target$Dataset[1]), "_", tolower(target$Code[1]), ")}\n",
      description_text
    ))
  } else {
...
variable_names <- unique(DEMIG_Policy$Code)
# Initialize a list to store the results
results_list_demig_policy <- list()
# Loop through the variable names and apply the function
for (variable_name in variable_names) {
  result <- hierarical_structure(DEMIG_Policy, variable_name)
  results_list_demig_policy[[variable_name]] <- result
}

# Define the file path where you want to save the LaTeX file
latex_file_path <- "DEMIG_Policy-cb-section.tex"  # Replace with your desired file path
# Open the LaTeX file for writing
file_conn <- file(latex_file_path, "w")
# Loop through the results and write them to the LaTeX file
for (variable_name in variable_names) {
  cat("\n", file = file_conn)
  if (!is.null(results_list_demig_policy[[variable_name]])) {
    cat(results_list_demig_policy[[variable_name]], file = file_conn)
  } else {
...
############################################################
# Get a vector of unique variable names from the 'Code' column
variable_names <- unique(IMPIC_2016$Code)
# Initialize a list to store the results
results_list_impic_2016 <- list()
# Loop through the variable names and apply the function
for (variable_name in variable_names) {
  result <- hierarical_structure(IMPIC_2016, variable_name)
  results_list_impic_2016[[variable_name]] <- result
}

# Define the file path where you want to save the LaTeX file
latex_file_path <- "IMPIC_2016-cb-section.tex"  # Replace with your desired file path
# Open the LaTeX file for writing
file_conn <- file(latex_file_path, "w")
# Loop through the results and write them to the LaTeX file
for (variable_name in variable_names) {
  cat("\n", file = file_conn)
  if (!is.null(results_list_impic_2016[[variable_name]])) {
    cat(results_list_impic_2016[[variable_name]], file = file_conn)
  } else {
    cat("No heading for the specified type.", file = file_conn)
  }
  cat("\n\n", file = file_conn)
...
tex_files <- c("migpol-cb-intro.tex",
               "DEMIG_Policy_introduction.tex", "demig_policy-cb-section.tex", 
               ...
               "impic_2016_introduction.tex", "impic_2016-cb-section.tex",
