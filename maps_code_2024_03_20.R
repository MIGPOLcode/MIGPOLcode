...
db_demig <- read_excel("MIGPOL_DEMIG_QUANTMIG.xlsx")
...
db_demig <- db_demig %>% rename_all(~gsub("^demig_quantmig_", "", .))
...
DEMIG_QUANTMIG <- db_demig %>% pivot_longer(!iso3, names_to = "Code", values_to = "percentages")
...
#### IMPIC 2024 ####
db_impic_2024 <- read_xlsx ("MIGPOL_IMPIC_2024.xlsx")
#All missing values are already coded as NA
#Remove variables not included in maps and those missing iso3 values
db_impic_2024 <- db_impic_2024 %>%
  select(-country_full_name, -iso2, -year) %>% filter(!is.na(iso3))
#Remove all impic_ additions in front of names 
db_impic_2024 <- db_impic_2024 %>% rename_all(~gsub("^impic_", "", .))
#Group by iso3, sum everything that is not missing and divide by .39.
# This code calculates the total number of observations of a variable by country over 39 years. 
# By dividing by .39, we get the percentage of years in which data was available. 
db_impic_2024 <- db_impic_2024 %>% group_by(iso3) %>%
  summarise(across(everything(), ~sum(!is.na(.))))
db_impic_2024 <- db_impic_2024 %>% mutate(across(-iso3, ~./0.39))
#Changes format to have iso3, Code and percentages as columns
IMPIC_2024 <- db_impic_2024 %>% pivot_longer(!iso3, names_to = "Code", values_to = "percentages")
...
#### IMPIC Antidiscrimination ####
db_impic_antidiscrimination <- read_xlsx ("MIGPOL_IMPIC_Antidiscrimination.xlsx")
#Remove variables not included in maps and those missing iso3 values
db_impic_antidiscrimination <- db_impic_antidiscrimination %>%
  select(-country_full_name, -iso2, -year) %>% filter(!is.na(iso3))
#Remove all impic_ additions in front of names 
db_impic_antidiscrimination <- db_impic_antidiscrimination %>% rename_all(~gsub("^impic_", "", .))
#Group by iso3, sum everything that is not missing and divide by .19.
# This code calculates the total number of observations of a variable by country over 19 years. 
# By dividing by .19, we get the percentage of years in which data was available. 
db_impic_antidiscrimination <- db_impic_antidiscrimination %>% group_by(iso3) %>%
  summarise(across(everything(), ~sum(!is.na(.))))
db_impic_antidiscrimination <- db_impic_antidiscrimination %>% mutate(across(-iso3, ~./0.19))
#Changes format to have iso3, Code and percentages as columns
IMPIC_Antidiscrimination <- db_impic_antidiscrimination %>% pivot_longer(!iso3, names_to = "Code", values_to = "percentages")



#### IMPIC Antidiscrimination raw data ####
db_impic_antidiscrimination_rawdata <- read_xlsx ("MIGPOL_IMPIC_Antidiscrimination_RawData.xlsx")
# Replace missing values with NA 
db_impic_antidiscrimination_rawdata[db_impic_antidiscrimination_rawdata == -8] <- NA
db_impic_antidiscrimination_rawdata[db_impic_antidiscrimination_rawdata == -9] <- NA
#Remove variables not included in maps and those missing iso3 values
db_impic_antidiscrimination_rawdata <- db_impic_antidiscrimination_rawdata %>%
  select(-country_full_name, -iso2, -year) %>% filter(!is.na(iso3))
#Remove all impic_ additions in front of names 
db_impic_antidiscrimination_rawdata <- db_impic_antidiscrimination_rawdata %>% rename_all(~gsub("^impic_", "", .))
#Group by iso3, sum everything that is not missing and divide by .19.
# This code calculates the total number of observations of a variable by country over 19 years. 
# By dividing by .19, we get the percentage of years in which data was available. 
db_impic_antidiscrimination_rawdata <- db_impic_antidiscrimination_rawdata %>% group_by(iso3) %>%
  summarise(across(everything(), ~sum(!is.na(.))))
db_impic_antidiscrimination_rawdata <- db_impic_antidiscrimination_rawdata %>% mutate(across(-iso3, ~./0.19))
#Changes format to have iso3, Code and percentages as columns
IMPIC_Antidiscrimination_RawData <- db_impic_antidiscrimination_rawdata %>% pivot_longer(!iso3, names_to = "Code", values_to = "percentages")
...
# Define the create_maps function with an additional argument for dataframe name
  ...
  # List of data frames
  data_frames_list <- list(DEMIG_QUANTMIG, GLOBALCIT_Country_Year, IMISEM, IMPIC_2024, 
                           IMPIC_Antidiscrimination, IMPIC_Antidiscrimination_RawData,
                           IMPIC_Political_Rights, IMPIC_RawData,
                           MIPEX)  # Add your data frames here
  
  # Names of the data frames
  data_frames_names <- c("DEMIG_QUANTMIG", "GLOBALCIT_Country_Year", "IMISEM", "IMPIC_2024", 
                         "IMPIC_Antidiscrimination", "IMPIC_Antidiscrimination_RawData",
                         "IMPIC_Political_Rights", "IMPIC_RawData",
                         "MIPEX")  # Add your data frame names here
